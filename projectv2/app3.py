...
# Yep, some repeats. Happens when you copy-paste and forget to clean up. We’ve all been there.
...
# Note: we're using SQLite because, hey, it's easy and doesn't complain much.
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///db.sqlite'
app.config['SECRET_KEY'] = 'mysecretkey'  # Top secret (not really)
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # Nobody's got time for unnecessary signals
...
# Let's get old school with SQLAlchemy ORM for our users
class User(db.Model, UserMixin):
    id = db.Column(db.Integer, primary_key=True)
    username = db.Column(db.String(150), unique=True, nullable=False)  # 150 should be plenty unless you're Elon
    email = db.Column(db.String(120), unique=True, nullable=False)  # Emails need to be unique, duh
    password = db.Column(db.String(200), nullable=False)
    is_admin = db.Column(db.Boolean, default=False)  # Toggle this to True and boom: admin powers

    def __repr__(self):
        return f"<User email='{self.email}'>"  # Not really necessary, but nice for debugging

# Track all the wild detections going on
class DetectionLog(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    date = db.Column(db.String(20), nullable=False)  # yeah, it's a string... don't judge
    time = db.Column(db.String(20), nullable=False)
    used_resource = db.Column(db.String(50), nullable=False)  # Could be camera, image, psychic vision, etc.
...
# Because SQLite really likes to keep counting, we reset those sequences like rebels
def reset_auto_increment():
    try:
        ...
        print(f"Hmm, could not reset auto-increments: {e}")  # Not the end of the world
...
@app.route('/')
def index():
    # Nothing fancy here, just the welcome mat
...
def login_signup():
    if request.method == 'POST':
        ...
        if confirm_password:  # Ah, this must be a signup attempt
            if user:
            ...
            # Automatically grant admin status to first lucky user
...
def api_delete_multiple_users():
    ...
    try:
        # Nuke 'em. All at once. Like a boss.
...
def logout():
    ...
    session.clear()  # Wipe it all, just to be sure
...
AUDIO_FOLDER = 'static/audio'  # Guess who might scream if there's too many people

app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['AUDIO_FOLDER'] = AUDIO_FOLDER

# Check if folders exist; if not, go full IKEA and build them
...
max_people_count = 0
stop_feed = False
heatmap_data = []  # Basically breadcrumbs for people movement
...
def initialize_yolo():
    try:
        ...
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"🤖 Model missing at {model_path} — did someone delete it?")

        model = YOLO(model_path)
        model.fuse()  # Optional turbo mode
        return model
    except Exception as err:
...
def detect_people(frame, model):
    ...
    if frame is None or model is None:
        print("No frame or model... can't detect anything from nothing.")
        return [], []
    ...
    try:
        ...
        if temp_heatpoints:
            ...
            # Keep the history tiny, just the last 10 — we're not the NSA
        ...
        for frame in heatmap_data:
            for (x, y) in frame:
                points.append({
                    'x': x,
                    'y': y,
                    'frame_width': 640,   # lol hardcoded
                    'frame_height': 480
                })
        return jsonify(points)
    except Exception as err:
        app.logger.error(f"Heatmap error: {str(err)}")
        return jsonify([]), 200  # Return an empty map, not chaos
...
def generate_frames(video_source=None):
    ...
    @contextmanager
    def suppress_stderr():
        # Because OpenCV loves yelling warnings no one asked for
...
def check_audio():
    ...
    if os.path.exists(audio_path):
        return f"🔊 Yup, audio is alive and well at {audio_path}"
    else:
        return f"🤷‍♂️ Uh oh. No alert.mp3 found at {audio_path}"
...
def stop_feed_func():
    ...
    stop_feed = True
    heatmap_data = []  # reset for the next chaos session
    socketio.emit('feed_stopped')  # FYI to the client

    import time
    time.sleep(0.5)  # half a second pause. Feels right.

    used = session.get('used_resource', 'Live Feed')
    if used == 'live':
        used = 'Live-Feed'  # Consistency is a virtue
...
def background_people_count():
    ...
    while not stop_feed:
        ...
        socketio.sleep(1)  # don't go too fast
...
# --- DELETE SINGLE LOG ENTRY (with respect) ---

@app.route('/delete_log/<int:log_id>')
@login_required
def delete_log(log_id):
    db.session.execute('PRAGMA foreign_keys = OFF;')  # Why? Because foreign keys are clingy
    try:
        ...
        db.session.execute('PRAGMA foreign_keys = ON;')  # Put the rules back
        db.session.commit()
...
def ensure_admin_exists():
    with app.app_context():
        if User.query.count() == 0:
            # First user gets the golden ticket
            root_admin = User(
                username='admin',
